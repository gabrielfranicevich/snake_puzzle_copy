...
// The parser adds them in order: head first, then all M segments (left→right top→bottom), then tail last.
// Trap tiles use 'X'.
...
void loadLevelData(int idx, GameState& state) {
    ...
    for (int gy = 0; gy < state.h; gy++) {
        ...
        for (int gx = 0; gx < state.w; gx++) {
            char c = (gx < rl) ? row[gx] : ' ';
            switch(c) {
                case '=': state.at(gx, gy) = T::Floor;  break;
                case 'A': state.at(gx, gy) = T::Apple;  state.apples++; break;
                case 'P': state.at(gx, gy) = T::Portal; break;
                case '#': state.at(gx, gy) = T::Box;    break;
                case 'X': state.at(gx, gy) = T::Trap;   break;
                case 'H': head = {gx, gy}; break;
                case 'M': mids.push_back({gx, gy}); break;
                case 'B': extra.push_back({gx, gy}); break;
                case ' ':
                case '.': break; // Void space
            }
        }
    }
    ...
    if (head.x >= 0) state.snake.push_back(head);
    for (auto& m : mids)  state.snake.push_back(m);
    for (auto& b : extra) state.snake.push_back(b);
}
