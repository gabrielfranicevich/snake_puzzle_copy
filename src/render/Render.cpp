...
Renderer::Renderer()
    : m_prog(0), m_vao(0), m_vbo(0), m_ebo(0), m_ox(0), m_texApple(0),
      m_texBlock(0), m_texTrap(0), m_texPortal(0), m_texSnakeHead(0),
      m_texSnakeMid(0), m_texSnakeTail(0) {}
...
void Renderer::init(int width, int height) {
  ...
  m_texSnakeHead = loadSVGTexture("assets/snake/head.svg", 256, 256);
  m_texSnakeMid = loadSVGTexture("assets/snake/mid.svg", 256, 256);
  m_texSnakeTail = loadSVGTexture("assets/snake/tail.svg", 256, 256);
}
...
// Draw a square centered at (cxp,cyp), with size sz x sz, rotated by angle
// (radians)
void Renderer::dRot(float cxp, float cyp, float w, float h, float angle,
                    const Opt &o) {
  // Build MVP: translate to center, rotate, scale, translate back so quad
  // [0,1]^2 is centered at origin
  glm::mat4 m = glm::translate(glm::mat4(1.0f), {cxp, cyp, 0.0f});
  m = glm::rotate(m, angle, {0.0f, 0.0f, 1.0f});
  m = glm::scale(m, {w, h, 1.0f});
  m = glm::translate(m, {-0.5f, -0.5f, 0.0f}); // center the unit quad
  glm::mat4 mvp = m_proj * m;
  glUseProgram(m_prog);
  glUniformMatrix4fv(m_lMVP, 1, GL_FALSE, glm::value_ptr(mvp));
  glUniform4fv(m_lCol, 1, glm::value_ptr(o.c));
...
float Renderer::dStr(const char *s, float px, float py, float sc,
                     glm::vec4 col) {
  float x = px;
  for (; *s; s++) {
    if (*s == ' ') {
...
static const glm::vec4 SKY{0.53f, 0.81f, 0.92f, 1},
    CITY{0.47f, 0.67f, 0.78f, 0.55f}, SNK_B{0.22f, 0.68f, 0.12f, 1},
    SNK_D{0.14f, 0.48f, 0.07f, 1}, SNK_EW{0.96f, 0.96f, 0.96f, 1},
    SNK_EP{0.35f, 0.25f, 0.45f, 0.9f}, BOX_F{0.65f, 0.45f, 0.20f, 1},
    BOX_S{0.40f, 0.25f, 0.08f, 1}, HUD_PL{0.25f, 0.25f, 0.32f, 0.92f},
    HUD_TF{0.96f, 0.96f, 0.96f, 1}, HUD_BT{0.30f, 0.30f, 0.38f, 0.88f},
    WIN_BG{0.76f, 0.50f, 0.26f, 1}, WIN_IB{0.96f, 0.90f, 0.76f, 1},
    WIN_ST{0.98f, 0.82f, 0.12f, 1}, WIN_SE{0.30f, 0.30f, 0.36f, 1},
    WIN_GN{0.22f, 0.78f, 0.22f, 1}, WIN_TN{0.45f, 0.22f, 0.06f, 1},
    RIB{0.80f, 0.14f, 0.14f, 1}, DIM{0.40f, 0.46f, 0.56f, 1},
...
void Renderer::drawCity() {
  struct Bld {
    float x, w, h;
      ...
      {0, 55, 130},   {40, 38, 180},  {80, 62, 110},  {130, 45, 160},
      {165, 70, 200}, {220, 40, 140}, {255, 55, 170}, {300, 48, 130},
      {340, 65, 190}, {395, 42, 155}, {430, 60, 120}, {480, 75, 210},
      {540, 50, 145}, {580, 44, 175}, {615, 68, 130}, {670, 55, 190},
      {715, 40, 155}, {748, 72, 215}, {808, 50, 140}, {848, 60, 170},
      {895, 48, 125}, {935, 65, 200}, {987, 43, 155},
  };
  Opt o;
  o.c = CITY;
...
void Renderer::drawPortal(int gx, int gy, float t) {
  ...
  if (m_texPortal) {
    ...
    Opt glow;
    glow.c = {0.45f, 0.30f, 0.80f, 0.5f};
    glow.r = 1.f;
    dC(gx, gy, sz * 1.15f, glow);

    // SVG vortex — true clockwise physical rotation based on time
    Opt o;
    o.c = {1, 1, 1, 1};
    o.tex = m_texPortal;
    ...
    Opt ot;
    ot.c = {0.45f, 0.30f, 0.80f, 0.9f};
    ot.r = 1.f;
    ot.t = t;
    dC(gx, gy, sz * pulse, ot);
    Opt in;
    in.c = {0.08f, 0.08f, 0.16f, 1};
    in.r = 1.f;
    in.fx = 1;
    in.t = t;
    dC(gx, gy, sz * 0.72f, in);
    Opt co;
    co.c = {0.75f, 0.60f, 1.0f, 0.7f};
    co.r = 1.f;
...
// angle: 0=right, pi/2=down, pi=left, -pi/2=up (OpenGL Y-down, so +Y is down)
static float segAngle(float dx, float dy) {
  // dx,dy is direction vector (grid units, Y grows downward)
  if (dx > 0.5f)
    return 0.0f; // facing right
  if (dx < -0.5f)
    return (float)M_PI; // facing left
  if (dy > 0.5f)
    return (float)M_PI * 0.5f; // facing down
  return -(float)M_PI * 0.5f;  // facing up
}
...
void Renderer::drawSnakeSegmentF(float gx, float gy, float sz, glm::vec4 col,
  ...
  if (isHead)
    segTex = m_texSnakeHead;
  if (isTail)
    segTex = m_texSnakeTail;
  ...
  if (segTex) {
    ...
    sh.c = {0, 0, 0, 0.20f};
    sh.r = 1.f;
    dRot(cx + 3, cy + 4, sz + 8, sz + 8, angle, sh);
    // Draw the SVG rotated to face the direction of travel
    Opt o;
    o.c = {1, 1, 1, 1};
    o.tex = segTex;
    dRot(cx, cy, sz, sz, angle, o);
  } else {
    // Fallback procedural
    Opt sh;
    sh.c = {0, 0, 0, 0.17f};
...
void Renderer::drawStar(float cx2, float cy2, float r, bool filled) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    o.c = col;
    dR(cx2 + r * cosf(a) - r * .38f, cy2 + r * sinf(a) - r * .38f, r * .76f,
       r * .76f, o);
  }
...
void Renderer::drawButton(float px, float py, float w, float h, glm::vec4 bg,
...
void Renderer::renderFrame(const GameState &state, int currentLevel,
                           int totalLevels, float time) {
  ...
  // ── HUD ──────────────────────────────────────────────────────────────────
  Opt hb;
  hb.c = {0.86f, 0.92f, 0.96f, 0.55f};
  dR(0, 0, (float)m_W, (float)HUD_H, hb);
  Opt sep;
  sep.c = {0.55f, 0.70f, 0.80f, 0.4f};
  ...
  {
    char buf[16];
    snprintf(buf, sizeof(buf), "%d", state.moves);
    float sc = 2.5f;
    float tw = strW(buf, sc);
  ...
  for (int gy = 0; gy < state.h; gy++)
    for (int gx = 0; gx < state.w; gx++)
      if (state.at(gx, gy) == T::Floor)
        drawTile(gx, gy);

  // 2. Trap tiles (SVG). Draw unconditionally (even if covered by a box or
  // snake)
  for (int gy = 0; gy < state.h; gy++) {
    for (int gx = 0; gx < state.w; gx++) {
      if (state.at(gx, gy) == T::Trap || state.trapMask[gy * state.w + gx]) {
        // Base points to nearest floor block.
        // SVG base is drawn at the bottom (angle 0).
        float angle = 0.0f;
        if (state.safeAt(gx, gy + 1) == T::Floor) {
          angle = 0.0f; // Base down, spikes up
        } else if (state.safeAt(gx - 1, gy) == T::Floor) {
          angle = (float)M_PI / 2.0f; // Base left, spikes right (>)
        } else if (state.safeAt(gx + 1, gy) == T::Floor) {
          angle = -(float)M_PI / 2.0f; // Base right, spikes left (<)
        } else if (state.safeAt(gx, gy - 1) == T::Floor) {
          angle = (float)M_PI; // Base up, spikes down
        }
        drawTrap(gx, gy, angle);
      }
    }
  }
  ...
  for (int gy = 0; gy < state.h; gy++)
    for (int gx = 0; gx < state.w; gx++)
      if (state.at(gx, gy) == T::Box)
        drawBox(gx, gy);

  // 4. Portal / vortex (SVG, clockwise)
  for (int gy = 0; gy < state.h; gy++)
    for (int gx = 0; gx < state.w; gx++)
      if (state.at(gx, gy) == T::Portal)
        drawPortal(gx, gy, time);

  // 5. Apples (SVG)
  for (int gy = 0; gy < state.h; gy++)
    for (int gx = 0; gx < state.w; gx++)
      if (state.at(gx, gy) == T::Apple)
        drawApple(gx, gy, time);
  ...
  for (int i = sLen - 1; i >= 0; i--) {
    ...
    if (pLen > 0 && mt < 1.0f) {
      if (sLen == pLen) {
        ...
        if (i == 0)
          prev = state.prevSnake[0];
        else
          prev = state.prevSnake[i - 1];
      } else {
        // Shrank (hit trap)
        if (i < pLen)
          prev = state.prevSnake[i];
    ...
    // If x jumped by more than 1 (meaning it wrapped the infinite map
    // horizontally), we DO NOT interpolate across the screen. Just snap.
    if (std::abs(curr.x - prev.x) <= 1 && std::abs(curr.y - prev.y) <= 1 &&
        mt < 1.0f) {
    ...
    // For tail: use direction from this segment to the one ahead of it
    // For body: use direction to the next segment ahead
    float angle = 0.0f;
    {
      float dx, dy;
      if (isHead) {
        // Head faces away from the nearest body segment (snake[1] → snake[0])
        if (sLen > 1) {
          dx = (float)(state.snake[0].x - state.snake[1].x);
          dy = (float)(state.snake[0].y - state.snake[1].y);
        } else {
          // Single-segment snake: fall back to last move direction
          dx = (float)state.lastDir.x;
          dy = (float)state.lastDir.y;
        }
      } else {
        // Body / tail: faces toward the next segment ahead (lower index)
        dx = (float)(state.snake[i - 1].x - curr.x);
        dy = (float)(state.snake[i - 1].y - curr.y);
      }
      angle = segAngle(dx, dy);
    }

    float rawCx = vx;
    float rawCy = vy;
    drawSnakeSegmentF(rawCx, rawCy, sz, col, isHead, isTail, angle, time);
  }

  // ── Footer ───────────────────────────────────────────────────────────────
  Opt bb;
  bb.c = {0.86f, 0.92f, 0.96f, 0.55f};
  dR(0, (float)(m_H - BOT_H), (float)m_W, (float)BOT_H, bb);
  Opt bsep;
  bsep.c = {0.55f, 0.70f, 0.80f, 0.40f};
  ...
  if (state.dead) {
    ...
    // Dark red border
    Opt bd;
    bd.c = {0.50f, 0.04f, 0.04f, alp};
    bd.r = 0.18f;
    dR(ppx - 4, ppy - 4, pw + 8, ph + 8, bd);
    // Panel
    Opt panel;
    panel.c = {0.18f, 0.04f, 0.04f, alp};
    panel.r = 0.18f;
    dR(ppx, ppy, pw, ph, panel);
    // Red ribbon accent
    Opt rib2;
    rib2.c = {0.72f, 0.08f, 0.08f, alp};
    rib2.r = 0.10f;
  ...
  if (state.won) {
    ...
    Opt ov;
    ov.c = {0.f, 0.f, 0.f, 0.62f * alp};
    dR(0, 0, (float)m_W, (float)m_H, ov);

    float pw = 360, ph = 260;
    float ppx = (m_W - pw) * .5f, ppy = (m_H - ph) * .5f;
    Opt bd;
    bd.c = {0.40f, 0.22f, 0.06f, alp};
    bd.r = 0.18f;
    dR(ppx - 4, ppy - 4, pw + 8, ph + 8, bd);
    Opt wood;
    wood.c = WIN_BG;
    ...
    for (int i = 0; i < 3; i++)
      drawStar(sx + i * sr * 2.6f, sy, sr, i < state.stars);
    ...
    if (currentLevel == totalLevels - 1) {
      float sc = 2.5f;
      const char *fin = "ALL LEVELS DONE!";
