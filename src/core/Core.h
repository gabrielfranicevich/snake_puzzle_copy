...
struct GameState {
  ...
  V2 lastDir = {0, 0};
  // Permanent record of which tiles started as traps (never mutated after
  // load). Used to restore T::Trap when a box moves off a trap tile.
  std::vector<bool> trapMask;
